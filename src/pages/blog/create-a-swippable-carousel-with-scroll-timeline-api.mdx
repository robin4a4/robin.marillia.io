...
title: "Create a swippable carousel with the scroll timeline API"
...
import CarouselFirstExample from "./components/CarouselFirstExample.astro";
import CarouselSlideAnimationExample from "./components/CarouselSlideAnimationExample.astro";
import CarouselFinalExample from "./components/CarouselFinalExample.astro";
...
Unfortunately, for now, the polyfill prevents us to animate other elements than the element on which we set the `view-timeline` property (we need the `timeline-scope` property which is not currently supported in the polyfill, see [https://github.com/flackr/scroll-timeline/issues/123](https://github.com/flackr/scroll-timeline/issues/123). So we will have to use a little bit of javascript to animate our dots.

{/* <CarouselSlideAnimationExample /> */}
